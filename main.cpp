...
static uint8_t tensor_arena[tensorArenaSize];

static const tflite::Model*      tf_model       = nullptr;
static tflite::MicroInterpreter* tf_interpreter = nullptr;
static TfLiteTensor*             tf_input       = nullptr;
static TfLiteTensor*             tf_output      = nullptr;

static const float mean_temp = 25.0;
static const float std_temp  =  5.0;
static const float mean_hum  = 50.0;
static const float std_hum   = 20.0;

constexpr int time_steps    = 3;
constexpr int feature_id  = 2;

// 3 times: [t0, t1, t2][feature], feature: 0 = temperature, feature 1 = humidity
static float input_buffer[time_steps][feature_id] = { {0} };
...
void setup()
{
  ...
  InitWiFi();
  Wire.begin(SDA_PIN, SCL_PIN);   // Before any init of devices utilizing I2C such as LCD and DHT20
  ...
  tf_model = tflite::GetModel(model_tflite);
  ...
  static tflite::MicroInterpreter interpreter(tf_model, resolver, tensor_arena, tensorArenaSize, &micro_error_reporter);
  tf_interpreter = &interpreter;
  TfLiteStatus status = tf_interpreter->AllocateTensors();
  Serial.printf("AllocateTensors: %s\n", (status==kTfLiteOk?"OK":"ERR"));
...
void task_send_telemetry(void *pvParameters)
{
  while(1)
  {
    ...
    {
      for(int j = 0; j < feature_id; ++j)
        input_buffer[i][j] = input_buffer[i+1][j];
    }

    input_buffer[time_steps-1][0] = (temperature - mean_temp) / std_temp;
    input_buffer[time_steps-1][1] = (humidity    - mean_hum ) / std_hum;

    if(buffer_count < time_steps) ++buffer_count;

    if(buffer_count >= time_steps)
    {
      Serial.print("Buffer raw: ");
      for (int i = 0; i < time_steps; ++i)
        Serial.printf("[%.2f,%.2f] ", input_buffer[i][0], input_buffer[i][1]);
      Serial.println();

      for(int i = 0; i < time_steps; ++i)
      {
        for(int j = 0; j < feature_id; ++j)
          tf_input->data.f[i * feature_id + j] = input_buffer[i][j];
      }

      Serial.print("Model input: ");
      for (int idx = 0; idx < time_steps*feature_id; ++idx)
            Serial.printf("%.2f ", tf_input->data.f[idx]);
      Serial.println();

      tf_interpreter->Invoke();
      float predicted_n_temp  = tf_output->data.f[0];
      float predicted_n_humid = tf_output->data.f[1];
      float predicted_temp = predicted_n_temp * std_temp + mean_temp;
      float predicted_humid  = predicted_n_humid  * std_hum  + mean_hum;

      Serial.printf("Prediction Model: Predicted Temp = %.3f, Predicted Hum = %.3f\n", predicted_temp, predicted_humid);
      tb.sendTelemetryData("predicted_temp", predicted_temp);
      tb.sendTelemetryData("predicted_humid", predicted_humid);
    }
